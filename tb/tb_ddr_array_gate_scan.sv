module tb_ddr_array_gate_scan #(
    ...
    rv32ima_DdrArrayProbe #(.GATE_SCAN(1), .SAME_DATA(SAME_DATA), .EARLY_GATE(EARLY_GATE), .PHASE_SCAN(PHASE_SCAN)) dut (
        ...
        .o_read(read_gate), .o_sel(sel), .o_hold(), .o_odt(),
    ...
    task automatic run_case(input bit stale, input bit corrupt=0);
        @(negedge clk); rst=1; enable=0; stale_second=stale; corrupt_lane=corrupt;
        repeat (2) @(negedge clk);
        rst=0; enable=1;
        for (int n=0; n<300; n++) begin
            @(negedge clk);
            if (done) begin
                if (found==(stale || corrupt) || gate0!=(stale || corrupt ? 8'h00 : 8'h08) ||
                    gate1!=(stale || corrupt ? 8'h00 : 8'h20) ||
