module tb_ddr_trained_array;
    ...
    wire [7:0] offsets;
    logic [127:0] data = 0, data_pipe[2], memory[2];
    logic [1:0] valid = 0, valid_pipe[2];
    logic [255:0] stream;
    int reads = 0, writes = 0, pending = 0, mode = 0;
    rv32ima_DdrArrayProbe #(.READ_HOLD(0), .READ_SEL(6'h24)) array_probe (
    ...
    always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            if (pending > 0) begin
                ...
                if (pending == 1) begin data <= stream[255:128]; valid <= 3; end
                pending--;
            end
            if (cmd_valid && cmd == 3'b101) begin
                if (writes != 2) $fatal(1, "read before writes");
                stream = 0;
    ...
    initial begin
        for (int scenario = 0; scenario < 3; scenario++) begin
            ...
            if (found !== (scenario == 0) || reads != 2 || writes != 2)
                $fatal(1, "trained array scenario=%0d found=%b", scenario, found);
        end
        $display("DDR trained array PASS: eight-cycle PHY latency, lane offsets, stale/corrupt rejection");
        $finish;
    end
