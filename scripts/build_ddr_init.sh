#!/usr/bin/env bash
# Build an initialization-only bitstream in the pinned Gowin container.
set -euo pipefail
root="$(cd "$(dirname "$0")/.." && pwd)"
cd "$root"
out="$root/sim/fpga/ddr_init"
gowin="${GOWIN_HOME:-/opt/gowin/IDE}"
mkdir -p "$out"
veryl build > "$out/veryl.log" 2>&1
cat fpga/tang_primer_20k/ddr_phy_check.cst fpga/tang_primer_20k/ddr_probe_uart.cst > "$out/ddr_init.cst"
cd "$out"
env QT_QPA_PLATFORM=offscreen \
    LD_LIBRARY_PATH="/opt/gowin-runtime/usr/lib/x86_64-linux-gnu:/usr/lib/x86_64-linux-gnu:$gowin/lib" \
    "$gowin/bin/gw_sh" "$root/scripts/build_ddr_init.tcl" > gowin.log 2>&1 || {
    tail -60 gowin.log >&2
    exit 1
}
report=impl/pnr/ddr_init.rpt.txt
...
timing=impl/pnr/ddr_init_tr_content.html
...
echo "DDR init probe: $out/impl/pnr/ddr_init.fs"
