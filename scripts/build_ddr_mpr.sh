...
cd "$root"
out="$root/sim/fpga/ddr_mpr"
gowin="${GOWIN_HOME:-/opt/gowin/IDE}"
mkdir -p "$out"
veryl build > "$out/veryl.log" 2>&1
cat fpga/tang_primer_20k/ddr_phy_check.cst fpga/tang_primer_20k/ddr_probe_uart.cst > "$out/ddr_mpr.cst"
cd "$out"
...
report=impl/pnr/ddr_mpr.rpt.txt
timing=impl/pnr/ddr_mpr_tr_content.html
...
echo "DDR MPR probe: $out/impl/pnr/ddr_mpr.fs"
