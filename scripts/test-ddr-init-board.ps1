...
param(
    ...
    [Parameter(Mandatory = $true)]
    [string]$Port,
    [ValidateSet('DdrInit', 'DdrRead', 'DdrMpr', 'DdrMprDelay', 'DdrMprAlign', 'DdrArray', 'DdrArrayScan')]
    [string]$Mode = 'DdrInit'
)
...
try {
    $probeSeconds = if ($Mode -in @('DdrMprDelay', 'DdrMprAlign', 'DdrArray', 'DdrArrayScan')) { 6 } else { 4 }
