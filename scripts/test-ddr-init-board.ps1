...
param(
    ...
    [Parameter(Mandatory = $true)]
    [string]$Port,
    [ValidateSet('DdrInit', 'DdrRead', 'DdrMpr', 'DdrMprDelay', 'DdrMprAlign', 'DdrArray', 'DdrArrayScan', 'DdrArraySame')]
    [string]$Mode = 'DdrInit'
)
...
try {
    $probeSeconds = if ($Mode -in @('DdrMprDelay', 'DdrMprAlign', 'DdrArray', 'DdrArrayScan', 'DdrArraySame')) { 6 } else { 4 }
