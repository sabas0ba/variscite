...
param(
    ...
    [Parameter(Mandatory = $true)]
    [string]$Port,
    [ValidateSet('DdrInit', 'DdrRead', 'DdrMpr', 'DdrMprDelay', 'DdrMprAlign', 'DdrArray', 'DdrArrayScan', 'DdrArraySame', 'DdrArrayTrace', 'DdrArrayMatch', 'DdrArrayFlags')]
    [string]$Mode = 'DdrInit'
)
...
try {
    $probeSeconds = if ($Mode -in @('DdrMprDelay', 'DdrMprAlign', 'DdrArray', 'DdrArrayScan', 'DdrArraySame', 'DdrArrayTrace', 'DdrArrayMatch', 'DdrArrayFlags')) { 6 } else { 4 }
