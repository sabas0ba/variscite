#!/usr/bin/env bash
# Build the DQS READ gate probe in the pinned Gowin container.
set -euo pipefail
root="$(cd "$(dirname "$0")/.." && pwd)"
cd "$root"
out="$root/sim/fpga/ddr_read"
gowin="${GOWIN_HOME:-/opt/gowin/IDE}"
mkdir -p "$out"
veryl build > "$out/veryl.log" 2>&1
cat fpga/tang_primer_20k/ddr_phy_check.cst fpga/tang_primer_20k/ddr_probe_uart.cst > "$out/ddr_read.cst"
cd "$out"
env QT_QPA_PLATFORM=offscreen \
    LD_LIBRARY_PATH="/opt/gowin-runtime/usr/lib/x86_64-linux-gnu:/usr/lib/x86_64-linux-gnu:$gowin/lib" \
    "$gowin/bin/gw_sh" "$root/scripts/build_ddr_read.tcl" > gowin.log 2>&1 || {
    tail -60 gowin.log >&2
    exit 1
}
report=impl/pnr/ddr_read.rpt.txt
timing=impl/pnr/ddr_read_tr_content.html
...
echo "DDR READ gate probe: $out/impl/pnr/ddr_read.fs"
